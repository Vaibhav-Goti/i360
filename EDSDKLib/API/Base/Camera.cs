...
namespace EOSDigital.API
{
    ...
    public class Camera : IDisposable
    {
        ...
        public void Beep(bool isDisposed)
        {
        ...
        public void Beep()
        {
            ...
            ThreadPool.QueueUserWorkItem((state) =>
            {
                try
                {
                    MainThread.Invoke(() =>
                    {
                        SendCommand(CameraCommand.PressShutterButton, (int)MyMenuID.Beep);
                    });
                }
                catch (Exception ex) { if (!ErrorHandler.ReportError(this, ex)) throw; }
            });
        ...
        /// Only the PropertyIDs "AEModeSelect", "ISO", "VIS", "Av", "Tv", "MeteringMode" and "ExposureCompensation" are allowed.
        /// </summary>
        ...
        public CameraValue[] GetSettingsList(PropertyID propId)
        {
            ...
            if (propId == PropertyID.AEModeSelect || propId == PropertyID.ISO || propId == PropertyID.Av || propId == PropertyID.Tv
                || propId == PropertyID.WhiteBalance || propId == PropertyID.MeteringMode || propId == PropertyID.ExposureCompensation)
            {
